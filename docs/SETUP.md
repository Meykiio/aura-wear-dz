...
3. From **Project Settings → API**, copy:
   - Project URL
   - `anon` / publishable key (starts with `sb_publishable_` for new projects, or `eyJhbGci...` legacy JWT format)
...
- Validation triggers + RLS policies + seed catalog data + seed reviews
- Base GRANTs for anon, authenticated, and service_role roles (required by PostgREST)
...
- `VITE_SUPABASE_PUBLISHABLE_KEY` and `SUPABASE_PUBLISHABLE_KEY` → your publishable/anon key
...
- **`401 Unauthorized` from PostgREST** — missing base GRANT for the role. Run `supabase db push` to ensure migration `20260522193000` (anon/authenticated SELECT) and `20260522210000` (authenticated INSERT/UPDATE/DELETE) are applied.
...
- **Hydration mismatch warnings in console** — browser extensions inject attributes. These are suppressed via `suppressHydrationWarning` on `<body>` and `<video>`.
