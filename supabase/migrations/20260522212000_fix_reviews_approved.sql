-- Fix: seed reviews were inserted with is_approved=false because the
-- validate_review_trigger sets is_approved=false when auth.uid() is null.
-- Drop trigger, update to approved, re-create.

DROP TRIGGER IF EXISTS validate_review_trigger ON public.reviews;

UPDATE public.reviews SET is_approved = true WHERE id IN (
  '66df8d89-d841-45e1-acd4-52b10a96e66c',
  '0300a4a8-3521-4f3b-806a-d4041e05622a',
  'd6b1f283-49a1-47af-a251-723a762d5677',
  '1a286b78-ed0c-471e-bb88-b52c462b4850',
  '8952f308-569f-4f3b-9688-5d170c3f6b47',
  'db160eac-3296-46f2-96a7-65eb88283253',
  '0851be29-3f40-47f5-a023-1b3fb320aea3',
  'b4feed75-2b34-4479-a6f3-357e21674a7a'
);

-- Insert any that might be missing (since the original seed trigger blocked them)
